...
function getTests(root: Instance) {
	...
	assert(classIs(folder, "Folder"), "Could not find __tests__ folder in ReplicatedStorage.TS");
	return folder;
}

const setup = script.Parent?.WaitForChild("setup");
assert(setup !== undefined && classIs(setup, "ModuleScript"), "Could not find setup module script");
